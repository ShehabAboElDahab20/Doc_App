...
class ErrorHandler implements Exception {
  ...
  ErrorHandler.handle(dynamic error) {
    ...
    if (error is DioException) {
      debugPrint("Error handle statusCode: ${error.response?.statusCode}");
      debugPrint("Error handle index: ${error.type.index}");
      apiErrorModel = ApiErrorModel(
          arMessage:
              "${error.response?.data["ar_message"] ?? error.response?.data["message"] ?? error.response?.data["error"] ?? error.message}",
          message:
              "${error.response?.data["message"] ?? error.response?.data["ar_message"] ?? error.response?.data["error"] ?? error.message}",
          code: error.type.index);
    } else {
      // default error
